...
本地证据研究与创作助手，当前 v0.10.0。已实现文字/多图证据导入、本地模型分析、带引用研究、创意分镜、版本审核、研究六页摘要和可变页数发布稿导出及复盘输入。
...
新增 `research_topic` 将选出的3–5篇候选逐篇采集并研究，保存进度且失败即停止；`resume_topic_research` 显式续跑。研究后策划通过 `topic_job_id` 接入实际研究与来源依赖，搜索完成不再等于精读完成。见[主题精读流程](docs/17-topic-research.md)。
...
浏览器现增加页面级视频禁播及视频媒体/已识别的fetch资源拦截；搜索卡片明确显示视频标记时直接跳过，详情发现视频返回 `video_deferred`，不进入图片或研究处理。批量精读继续其余图文笔记，视频不计入完成数量。页面标记不明确时仍可能打开详情后才识别；本轮仅离线Chromium验证，未新增真实平台验收。
...
新增[采集审计修复](docs/20-capture-audit-fixes.md)：独立笔记身份及完整性检查、默认5条有依据的评论样本、阶段事件和安全异常定位、显式图片/评论续采。旧结果不能据此追溯为完整采集；真实页面兼容性仍需复测。

2026-09-18新增[五场景MCP审核](docs/21-mcp-five-scenario-audit.md)：修复真实后台浏览器动画帧停滞、Vue ref及轮播克隆兼容问题，17图续采与两主题各3篇纯采集通过，共7篇48图。按用户要求暂不进行本地模型测试；不代表完整精读或图片文字准确性验收通过。

2026-09-18另完成用户明确授权的[Codex助手pipeline复测](docs/22-assistant-pipeline-verification.md)：7篇48图形成21条带引用发现、两个3篇主题及一个独立单篇主题，3份六页策划预览。新增显式助手提交接口和SQLite11→12契约迁移；不是自动云端回退，未调用本地模型，未通过人工准确性验收或发布批准。

### 调用方 agent 与本地模型

MCP 现在提供 `caller_analysis` 和 `local_model_analysis` 两条路径。Codex 等调用方可通过分页证据、原始 ImageContent 和固定引用自行分析，再提交校验结果；本地模型路径保持可选。多客户端可连接同一个 loopback Streamable HTTP 服务。详见 [双路径接入说明](docs/23-agent-neutral-mcp.md)。
